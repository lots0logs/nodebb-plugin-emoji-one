...
function categorize(emojiDetails) {
  ...
  let list = _.each(emojiDetails, function (value, key) {
    ...
    let shortname = value.shortname.substring(1, value.shortname.length -1)
    let emoji = {id: shortname, file: key};
    if (value.shortname_alternates.length) {
      emoji.aliases = _.map(value.shortname_alternates, function (alias) { return alias.substring(1, alias.length - 1); });
    }
